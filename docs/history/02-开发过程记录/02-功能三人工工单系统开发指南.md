...
## 1. 新增文件与目录（建议）
- `app/backend/modules/shared/db.py` — 数据访问层（SQLite 封装）。
- `app/backend/modules/feature3/wechat_utils.py` — optional：access_token 缓存与 `send_customer_message`（非必需，H5 拉取方案可先仅记录答案）。
